/*TO DO:
 change all the functions in the program that have reference parameters or array parameter to use pointer parameters,and then modify the complete program accodingly.
 */


/* This practice requires you to complete a list of functions that work on integer arrays (use arrays as parameter)
   All functions' names are listed below, along with their respective functionalitities, you need to decide the each functions'return type as well as the paramters (use array)
...
//https://stackoverflow.com/questions/18907139/c-variable-expressions-for-defining-array-size
    //int SIZE2 = getSize();
...
/*passing by reference, actual argument variable is passed into the function (reference parameter as an alias of the actual argument variable), need to understand the difference between pass by value and pass by reference*/
{
...
void showArray(const int *array, int size)
{
    for(int i = 0; i < size; i++)
        //loop used to go over all the individual elements of the array
    {
        cout << *(array + i);//first output individual element in each iteration
...
void largestElement(int *array, int size)
{
    ...
    {
        if(*(array + i) > *(array + max_subscript)
            //if current element is bigger than current maximum value, then update
        {
            max_subscript = i; //asssign current element's subsript to current maximum value's subscript
            max_counter = 1;//reassign 1 to the occurance times of current maximum value, since now we have a new maximum value
        }
        else if(*(array + 1) == *(array + max_subscript))
            //if current element is equal than current maximum value, then update

        {
            max_subscript = i;//since we want to know the last occurance, we still need to update the maximum value's subscript to current value's subscript (which is bigger than max_subscript)
            max_counter += 1; //add 1 to occurance times of current maximum value
        }
    }
    cout << "largest element is " << *(array + max_subscript) << " and its last occurance is at subscipt " << max_subscript << endl;
